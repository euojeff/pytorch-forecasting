...
from pytorch_forecasting.models.nn import LSTM, GRU, MultiEmbedding
...
class GRUTemporalFusionTransformer(BaseModelWithCovariates):
    def __init__(
        self,
        hidden_size: int = 16,
        lstm_layers: int = 1,
        ...
        * non-linear variable processing in variable selection network can be shared among decoder and encoder
        ...
        # for hidden state of the lstm
        self.static_context_initial_hidden_gru = GatedResidualNetwork(
            input_size=self.hparams.hidden_size,
            hidden_size=self.hparams.hidden_size,
            output_size=self.hparams.hidden_size,
            dropout=self.hparams.dropout,
    ...
    def forward(self, x: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # LSTM
        # calculate initial state
        input_hidden = self.static_context_initial_hidden_gru(static_embedding).expand(
            self.hparams.lstm_layers, -1, -1
        )

        # run local encoder
        encoder_output, hidden = self.gru_encoder(
            embeddings_varying_encoder, input_hidden, lengths=encoder_lengths, enforce_sorted=False
        )

        # run local decoder
        decoder_output, _ = self.gru_decoder(
            embeddings_varying_decoder,
            hidden,
            lengths=decoder_lengths,
            enforce_sorted=False,
